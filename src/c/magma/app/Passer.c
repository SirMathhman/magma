...
public struct Passer {public static Result<Node, CompileError> pass(Node root){
	return beforePass(root).orElse(new Ok<>(root)).flatMapValue(Passer::passNodes).flatMapValue(Passer::passNodeLists).flatMapValue(inner ->afterPass(inner).orElse(new Ok<>(inner)));
}public static Optional<Result<Node, CompileError>> beforePass(Node node){
	return removePackageStatements(node).or(() ->renameToStruct(node));
	...
	return Streams.from(tuple.right()).map(Passer::pass).foldLeftToResult(new ArrayList<>(), Passer::foldElementIntoList).mapValue(list -> node.withNodeList(tuple.left(), list));
