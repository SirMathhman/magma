...
public struct TypeRule(String type, Rule rule) implements Rule {@Override
    public Result<NodeCompileError> parse(String input){return this.rule.parse(input)
    ...
    public Result<StringCompileError> generate(Node node){if(node.is(this.type)){return this.rule.generate(node)
