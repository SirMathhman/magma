...
public Result<String, CompileError> generate(Node node){final var before=node.findString(this.before).orElse("");final var after=node.findString(this.after).orElse("");return this.childRule.generate(node).mapValue(()->before+content+after);}}