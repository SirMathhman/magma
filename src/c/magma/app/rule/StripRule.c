...
) implements Rule {public StripRule(Rule childRule){
	this(childRule, "", "");}Result<Node, CompileError> parse(String input){
	return this.childRule.parse(input.strip());}Result<String, CompileError> generate(Node node){
