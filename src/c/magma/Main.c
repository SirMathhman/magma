...
struct Main {
	...
	void main(Slice<String> args){
		SOURCE_DIRECTORY.walk().mapValue(Main.filterPaths).match(Main.compileSources, Some.new).ifPresent(auto _lambda8_(auto error){
	...
	Option<Error> compileSource(Path source){
		...
		return ensureDirectory(targetParent).or(auto _lambda9_(){
	...
	String compile(String root){
		return splitAndCompile(root, StatementSplitter(), Main.compileRootMember);
	}
	String splitAndCompile(String input, Splitter splitter, Function<String, String> compiler){
		return splitter.split(input).stream().map(String.strip).filter(auto _lambda11_(auto value){
			return !value.isEmpty();
		}).<Option<StringBuilder>>foldLeft(None<>(), auto _lambda10_(auto output, auto stripped){
	...
	Option<StringBuilder> compileAndMerge(Splitter splitter, Function<String, String> compiler, Option<StringBuilder> output, String stripped){
		...
		if (output.isEmpty()) {
			...
			return output.map(auto _lambda12_(auto inner){
	...
	String compileRootMember(String rootSegment){
		...
		return compileToStruct("class", rootSegment).or(auto _lambda15_(){
			return compileToStruct("interface", rootSegment);
		}).or(auto _lambda14_(){
			return compileToStruct("record", rootSegment);
		}).orElseGet(auto _lambda13_(){
			return invalidate(CompileError("Unknown " + "root segment", rootSegment));
		});
	}
	Option<String> compileToStruct(String keyword, String rootSegment){
		...
		auto compiled = splitAndCompile(content, StatementSplitter(), Main.compileClassSegment);
		return Some<>("struct " + name + " {" + compiled + "\n}");
	}
	String invalidate(CompileError error){
	...
	String compileClassSegment(String classSegment){
		...
		return compileMethod(classSegment).match(auto _lambda16_(auto value){
		return value;
		}, auto _lambda17_(auto err){
		return invalidate(CompileError("class segment", classSegment, err));
		});
	}
	CompileError> compileMethod(String classSegment){
		Locator locator3 = FirstLocator("(");
            ...
            return split(beforeParamStart, locator2).flatMapValue(tuple2 -> {
                ...
                return split(beforeName, locator1).flatMapValue(tuple1 -> {
                    ...
                    return split(afterParamStart, locator).flatMapValue(tuple0 -> {
                        ...
                        final var outputParams = splitAndCompile(inputParams, new ValueSplitter(), value -> compileDefinition(value).orElseGet(() -> invalidate(new CompileError("Unknown " + "definition", value))));
	...
	CompileError> truncateLeft(String input, String prefix){
		if (input.startsWith(prefix)) {
			return Ok<>(input.substring(prefix.length()));
		}
	...
	String compileStatement(String statement, int depth){
		...
		if (statement.startsWith("else")) {
			...
			if (substring.startsWith("{") && substring.endsWith("}")) {
				...
				output = splitAndCompile(substring1, StatementSplitter(), auto _lambda18_(auto statement0){
		...
		auto depth1 = compileInitialization(statement, depth);
		if (depth1.isPresent()) {
			return depth1.unwrap();
		}
		...
		return compileDefinitionStatement(statement).or(auto _lambda21_(){
			return compilePostfix(statement, "--", depth);
		}).or(auto _lambda20_(){
			return compilePostfix(statement, "++", depth);
		}).orElseGet(auto _lambda19_(){
			return invalidate(CompileError("Unknown " + "statement", statement));
		});
	}
	Option<String> compileInitialization(String statement, int depth){
		Locator locator = FirstLocator("=");
		return split(statement, locator).findValue().flatMap(auto _lambda24_(auto tuple){
		auto beforeEquals = tuple.left();
		auto afterEquals = tuple.right();
		return truncateRight(afterEquals, ";").findValue().map(String.strip).flatMap(auto _lambda23_(auto stripped){
			return compileDefinition(beforeEquals).map(auto _lambda22_(auto definition){
		auto compiled1 = compileValue(depth, stripped);
	...
	Option<String> compileInvocation(int depth, String statement){
		...
		return findMatchingChar(substring, Main.streamReverseIndices, '(', ')', '(').map(auto _lambda26_(auto index){
		auto caller = substring.substring(0, index);
		auto substring1 = substring.substring(index + 1);
		auto compiled = splitAndCompile(substring1, ValueSplitter(), auto _lambda25_(auto value){
	...
	Stream<Integer> streamReverseIndices(String input){
		return HeadedStream<>(new LengthHead(input.length())).map(auto _lambda27_(auto index){
	...
	String compileValue(int depth, String input){
		return compileSymbol(input).or(auto _lambda43_(){
		...
		}).orElseGet(auto _lambda28_(){
			return invalidate(CompileError("Unknown " + "value", input));
		});
	}
	...
	Option<String> compileLambda(int depth, String input){
		...
		if (afterArrow.startsWith("{") && afterArrow.endsWith("}")) {
			...
			compiled = splitAndCompile(substring1, StatementSplitter(), auto _lambda44_(auto statement){
		...
		return maybeNames.map(auto _lambda46_(auto names){
		auto joinedNames = names.stream().map(auto _lambda45_(auto name){
			return "auto " + name;
	...
	Option<String> compileConstruction(int depth, String input){
		...
		return findMatchingChar(withoutEnd, Main.streamReverseIndices, '(', ')', '(').map(auto _lambda48_(auto index){
		...
		auto compiled = splitAndCompile(substring1, ValueSplitter(), auto _lambda47_(auto value){
			return compileValue(depth, value.strip());
		});
	...
	Option<String> compileDefinition(String input){
		...
		auto inputParamType1 = findMatchingChar(inputParamType, Main.streamReverseIndices, ' ', '>', '<').map(auto _lambda49_(auto index){
	...
	String compileType(String input){
		return compileVar(input).or(auto _lambda53_(){
			return compileArray(input);
		}).or(auto _lambda52_(){
			return compileGenericType(input);
		}).or(auto _lambda51_(){
			return compileSymbol(input);
		}).orElseGet(auto _lambda50_(){
			return invalidate(CompileError("Unknown " + "type", input));
		});
	}
	...
	Option<String> compileArray(String input){
		return truncateRight(input, "[]").findValue().map(auto _lambda54_(auto inner){
			return generateGeneric("Slice", compileType(inner));
		});
	}
	Option<String> compileGenericType(String input){
		return split(input, FirstLocator("<")).findValue().flatMap(auto _lambda56_(auto tuple){
		auto caller = tuple.left();
		auto withEnd = tuple.right();
		return truncateRight(withEnd, ">").findValue().map(auto _lambda55_(auto inputArgs){
		auto outputArgs = splitAndCompile(inputArgs, ValueSplitter(), Main.compileType);
	...
	CompileError> split(String input, Locator locator){
		return locator.locate(input).match(auto _lambda57_(auto index){
		auto left = input.substring(0, index);
		auto right = input.substring(index + locator.sliceLength());
		return Ok<>(Tuple<>(left, right));
		}, auto _lambda58_(){
		auto format = "Infix '%s' not present";
		auto message = format.formatted(locator.infix());
		return Err<>(CompileError(message, input));
		});
	}
	CompileError> truncateRight(String input, String suffix){
		if (input.endsWith(suffix)) {
			return Ok<>(input.substring(0, input.length() - suffix.length()));
		}
		return Err<>(CompileError("Suffix '" + suffix + "' not present", input));
	}
	Set<Path> filterPaths(Set<Path> paths){
		return paths.stream().filter(Path.isRegularFile).filter(auto _lambda59_(auto path){
