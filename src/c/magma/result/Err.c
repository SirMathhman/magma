...
struct Err<T, X>(X error) {
	...
	void findError(void* __ref__){
		struct Err<T, X>(X error) this = *(struct Err<T, X>(X error)*) __ref__;
		return Node[value=Node[value=Node[value=Optional].of](Node[value=Node[value=this].error])];
	}
	...
	void match(void* __ref__, Function<T, R> onOk, Function<X, R> onErr){
		struct Err<T, X>(X error) this = *(struct Err<T, X>(X error)*) __ref__;
		return Node[value=Node[value=Node[value=onErr].apply](Node[value=Node[value=this].error])];
	}
