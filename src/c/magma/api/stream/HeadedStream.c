...
public struct HeadedStream<T>(Head<T> head) implements Stream<T> {
	@Override    public Optional<T> foldLeft(((T, T) => T) folder){
		return this.head.next().map(initial -> foldLeft(initial, folder));
	}
	@Override    public <R> R foldLeft(R initial,  ((R, T) => R) folder){
		var current =initial;
		while(true){
	...
	@Override    public <R> Stream<R> map(((T) => R) mapper){
		return new HeadedStream<>(() ->this.head.next().map(mapper));
	}
	@Override    public <R, X> Result<R, X> foldLeftToResult(R initial,  ((R, T) => Result<R, X>) folder){
