...
public record PrefixRule(String prefix, Rule childRule) implements Rule {
    private Optional<Node> parse0(String input) {
        ...
        return childRule.parse(input.substring(prefix.length())).first().orElseThrow().findValue();
    }

    private Optional<String> generate0(Node node) {
        return childRule.generate(node).first().orElseThrow().findValue().map(value -> prefix + value);
    }
